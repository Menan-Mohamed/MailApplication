...
public class MailService {
    ...
    public void createEmail(String from, ArrayList<String> to, String subject, String body, int priority, List<Attachment> attachments) {
    ...
    private List<Mail> getMailsFromFolder(Folder folder) {
        ...
        folder.getFolderMailIds().forEach(mailId -> {
            Mail mail = getEmail(mailId); // Assume getEmail gets a Mail object by its ID
            if (mail != null) {
                mails.add(mail);
            }
        });
    ...
    public Folder sortFolder(User user, String folderName, String strategy) {
        ...
        sortStrategy.sort(folder); // Sort the folder based on the selected strategy
...
//      mailService.createEmail("user100@example.com", recipients, "caro", "Don't forget our meeting tomorrow!", 3);
